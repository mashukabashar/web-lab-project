<?php 
	...
	if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
		echo "<script>alert('Invalid event ID!'); window.location.assign('other_gal.php');</script>";
		exit();
	}
	...
	if(isset($_POST['submit']))
	{
		...
		if($q=mysqli_fetch_assoc($list))
		{
			...
			echo "<script>alert('Entertainment event not found!'); window.location.assign('other_gal.php');</script>";
			exit();
		}
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<?php } else { 
						echo "<div class='alert alert-danger'>Entertainment event not found!</div>";
					} ?>
					</form>
		</div> 
			</div>
